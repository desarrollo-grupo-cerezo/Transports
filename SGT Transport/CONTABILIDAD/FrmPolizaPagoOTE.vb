...
Public Class FrmPolizaPagoOTE
    ...
    Private Sub FrmPolizaPagoOTE_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            TPOLIZA.Text = "Póliza Pago OT Ext " & DateTime.Now.ToString("MMMM")
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaPagoOTE_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Póliza Pago OT Ext.")
    End Sub
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        Try
            SQL = String.Format("SELECT a.*, b.EstaCuadrada, b.TotalDebe, b.TotalHaber, Diferencia = b.TotalDebe - b.TotalHaber 
                                FROM VT_CTB_POLIZA_PAGO_OTEXT a
                                INNER JOIN (SELECT	Documento, IIF(SUM(cast(iif(Debe = '', '0.00', Debe) AS decimal(27, 6)))-SUM(cast(iif(Haber = '', '0.00', Haber) AS decimal(27, 6))) = 0, 1, 0) AS EstaCuadrada, 
			                                ...
			                                FROM VT_CTB_POLIZA_PAGO_OTEXT
			                                WHERE Orden BETWEEN 2 AND 8 AND SubOrden = 0  AND {0}
...
#End Region
                            ...
                            If Convert.ToInt32(dr("Orden")) > 1 And Convert.ToInt32(dr("Orden")) < 9 And Convert.ToInt32(dr("SubOrden")) = 0 Then

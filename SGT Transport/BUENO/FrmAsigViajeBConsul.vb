...
Public Class FrnAsigViajeBConsul
    ...
    End Sub
    Private Sub FrnAsigViajeBConsul_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub CARGAR_DATOS1()
        ...
        Try
            ...
            FgCarga.Cols(2).StarWidth = "*" 'CLAVE
            FgCarga.Cols(3).StarWidth = "3*" 'EMBALAJE
            FgCarga.Cols(4).StarWidth = "*" 'CLAVE
            FgCarga.Cols(5).StarWidth = "3*" 'CARGA
            FgCarga.Cols(6).StarWidth = "2*" 'QUE EL REMITENTE DICE CONTIENEN
            FgCarga.Cols(7).StarWidth = "*" 'PESO
            FgCarga.Cols(8).StarWidth = "*" 'VOLUMEN
            FgCarga.Cols(9).StarWidth = "*" 'Peso estimado
            FgCarga.Cols(10).StarWidth = "*" 'PEDIMENTO

            FgCarga.Cols(1).MinWidth = 50
            FgCarga.Cols(11).Visible = False
        ...
        End If
        If LtStatus.Text = "Cancelado" Or LtStatus.Text = "LIQUIDADA" Or SOLOCONSULTAR Then
            Mofifica_Remitente_destinatario = False
        End If
        ...
        Try
            ...
            If dr.Read Then
                ...
                If dr.ReadNullAsEmptyInteger("CALLE_FISCAL") = 1 Then
                    LtCalle1.Text = dr("CALLE1")
                    LtCalle2.Text = dr("CALLE2")
                End If
            ...
            Else
                ...
                'LtCVE_VIAJE.Tag = ""
                'Ltviaje2.Text = LtCVE_VIAJE.Text
                MsgBox("Viaje inexistente, verifique por favor")
            End If
            dr.Close()
    ...
    Sub DESPLEGAR_CARGAS(FCVE_VIAJE As String)
        ...
        Try
            Using cmd As SqlCommand = cnSAE.CreateCommand
                SQL = "SELECT C.CANT, C.EMBALAJE, E.DESCR AS DESCR_EMBALAJE, C.CARGA, G1.DESCR AS DESCR_CARGA, C.CONTIENE,
                    C.PESO, C.VOLUMEN, C.PESO_ESTIMADO, C.PEDIMENTO
                    FROM GCCARGA C
                ...
                Using dr As SqlDataReader = cmd.ExecuteReader
                    While dr.Read
                        FgCarga.AddItem("" & vbTab & dr("CANT") & vbTab & dr("EMBALAJE") & vbTab & dr("DESCR_EMBALAJE") & vbTab &
                                   dr("CARGA") & vbTab & dr("DESCR_CARGA") & vbTab & dr("CONTIENE") & vbTab &
    ...
    End Function
    Private Sub FrnAsigViajeBConsul_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        ...
        End Select
    End Sub
    Private Sub FrnAsigViajeBConsul_KeyPress(sender As Object, e As KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Sub GUARDAR_VIAJE()
        ...
        If DOC_NEW Then
            SQLJ = "INSERT INTO GCASIGNACION_VIAJE (CVE_VIAJE, FECHA, FECHA_CARGA, FECHA_DESCARGA, STATUS, TIPO_UNI, TIPO_VIAJE, 
                ...
                TIPO_FACTURACION, CALLE_FISCAL, CALLE1, CALLE2, FECHA_REAL_CARGA, FECHA_REAL_DESCARGA, KMS_VACIO) 
                VALUES (
                ...
                @CALLE_FISCAL, @CALLE1, @CALLE2, @FECHA_REAL_CARGA, @FECHA_REAL_DESCARGA, @KMS_VACIO)"
        Else
    ...
    Private Function GUARDAR_FACTURA() As Boolean
        ...
        Dim TIPO_VENTA_LOCAL As String = "F", SERIE_FMV As String, DESCR As String = "", Kfin As Integer, ExisteViaje As Boolean
    ...
    Private Sub BarSalir_Click(sender As Object, e As ClickEventArgs) Handles BarSalir.Click
        Me.Close()
    End Sub
    Private Sub FrnAsigViajeBConsul_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Me.Dispose()
    End Sub
    ...
    Private Sub BtnImportarExcel_Click(sender As Object, e As EventArgs) Handles BtnImportarExcel.Click
        ...
        Try
            For row = rw To sheet.Rows.Count - 1
                ...
                For k = 1 To Fg.Rows.Count - 1
                    ...
                    If IsNothing(Fg(k, 4)) Then
                        ...
                        If UNI_SAT.Trim.Length > 0 And DESCR.Trim.Length = 0 Then
                            If IO.File.Exists(Application.StartupPath & "\CatalogosSat\CAT_UNIDAD_PESO.xml") Then
                                doc.Load(Application.StartupPath & "\CatalogosSat\CAT_UNIDAD_PESO.xml")

                                Dim nodeList2 As XmlNodeList = doc.SelectNodes("/Catalogo/row[@Clave='" & UNI_SAT & "']")
                                If nodeList2.Count > 0 Then
                                    For Each node As XmlNode In nodeList2
                                        If Not IsNothing(node.Attributes("Descripcion")) Then
                                            Fg(k, 4) = node.Attributes("Descripcion").Value
                                        End If
                                    Next
                                End If
                            End If
    ...
    Sub CALCULO_ESCENARIO3_PAGOS_PARCIALES()
        ...
        End Try
    End Sub
    Private Sub TSUB_TOTAL_ValueChanged(sender As Object, e As EventArgs) Handles TSUB_TOTAL.ValueChanged
        Try
            If ACEPTA_CALCULO Then
            End If
        Catch ex As Exception
            Bitacora("416. " & ex.Message & vbNewLine & ex.StackTrace)
            MsgBox("416. " & ex.Message & vbNewLine & ex.StackTrace)
            ...
            Select Case TAB1.SelectedIndex
                ...
                Case 5 'IMPORTES
                    ...
                    Else
                        ...
                        If LtTimbrado.Text = "TIMBRADO" Or LtTimbrado.Text = "FACTURADO" Or LtStatus.Text = "Cancelado" Or LtStatus.Text = "LIQUIDADA" Or SOLOCONSULTAR Then
                            Efecto = False
                        Else
                            Efecto = True
                        End If
        ...
        Try
            If ENTRAM Then
                ...
                If e.KeyCode = Keys.Left Then
                    Label65.Text = TXT.Text.Length
                    BtnCopiarCargas.Text = TXT.SelectionStart
    ...
    Private Sub TXTN_KeyDown(sender As Object, e As KeyEventArgs) Handles TXTN.KeyDown
        Try
            ...
            If ENTRAM Then
                ENTRAM = False
                If e.KeyCode = Keys.Left Then
                    Label65.Text = TXTN.Text.Length
                    BtnCopiarCargas.Text = TXTN.SelectionStart
    ...
    Private Sub Fg_BeforeEdit(sender As Object, e As RowColEventArgs) Handles Fg.BeforeEdit
        Try
            ...
            If ENTRAM Then
                ...
                Else
                    If IsMatPeligroso Then
                        If e.Col = 8 Or e.Col = 9 Or e.Col = 10 Or e.Col = 11 Or e.Col = 12 Or e.Col = 13 Then
                            e.Cancel = True
                        End If
                    End If
                End If
    ...
    Private Sub TABONO_NETO_Validated(sender As Object, e As EventArgs) Handles TABONO_NETO.Validated
        ...
        End Try
    End Sub
    Private Sub TFOLIO_VIAJE_KeyDown(sender As Object, e As KeyEventArgs)
        Try
            If e.KeyCode = 13 Then
                TCVE_MON.Focus()
            End If
    ...
    Private Sub BtnPaisAdenda_Click(sender As Object, e As EventArgs) Handles BtnPaisAdenda.Click
        ...
        End Try
    End Sub
    Private Sub TFOLIO_VIAJE_PreviewKeyDown(sender As Object, e As PreviewKeyDownEventArgs)
        Try
            If e.KeyCode = 13 Then
                TCVE_MON.Focus()
            End If
        ...
        Try
            If ENTRAM Then
                ...
                If e.KeyCode = Keys.Left Then '<---------
                    ...
                    Label65.Text = TXTNC.Text.Length
                    BtnCopiarCargas.Text = TXTNC.SelectionStart

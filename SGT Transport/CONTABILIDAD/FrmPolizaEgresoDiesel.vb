...
Public Class FrmPolizaEgresoDiesel
    ...
    Sub CARGAR_DATOS1()
        ...
        Try
            ...
            tPOLIZA.Text = "Poliza egresos diesel Fecha " & DateTime.Now.ToShortDateString
        Catch ex As Exception
            MsgBox("13. " & ex.Message & vbNewLine & ex.StackTrace)
            Bitacora("13. " & ex.Message & vbNewLine & ex.StackTrace)
        End Try
    End Sub
    Private Sub FrmPolizaEgresoDiesel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaEgresoDiesel_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Poliza egresos diesel")
    End Sub
    ...
    Private Sub BtnFiltrar_Click(sender As Object, e As EventArgs) Handles BtnFiltrar.Click
        Try
            Dim REFER As String, NO_FACTURA As String
            FgF.Rows.Count = 1
            Fg.Rows.Count = 1
            FgF.SetCellCheck(0, 1, CheckEnum.Unchecked)
            ...
            Using cmd2 As SqlCommand = cnSAE.CreateCommand
                SQL = "SELECT D.REFER, M.NO_FACTURA, CLAVE, NOMBRE
                    FROM PAGA_DET" & Empresa & " D 
                    LEFT JOIN PAGA_M" & Empresa & " M ON M.REFER = D.REFER
                    LEFT JOIN PROV" & Empresa & " P ON P.CLAVE  = M.CVE_PROV
                    WHERE 
                    D.FECHA_APLI >= '" & FSQL(F1.Value) & "' AND D.FECHA_APLI <= '" & FSQL(F2.Value) & "' AND 
                    EXISTS (SELECT REFER_PAGA_M FROM GCCONCI_VALES_COMBUS WHERE CVE_DOC = M.NO_FACTURA AND CVE_PROV = M.CVE_PROV)"
                ...
                Using dr2 As SqlDataReader = cmd2.ExecuteReader
                    While dr2.Read
                        ...
                        FgF.AddItem("" & vbTab & "" & vbTab & REFER & vbTab & NO_FACTURA & vbTab &
                                    "(" & dr2("CLAVE").ToString.Trim & ") " & dr2("NOMBRE") & vbTab & dr2("CLAVE"))
                    End While
                End Using 'PAFA_DET
            End Using
        Catch ex As Exception
        ...
        End Try
    End Sub
    Private Sub BtnFiltrarUtilitarios_Click(sender As Object, e As EventArgs) 
        Try
            tPOLIZA.Text = "Poliza egresos diesel utilitarios Fecha " & DateTime.Now.ToShortDateString
            ...
            MOV = "UTILITARIOS"
            SQL = "SELECT GV.FOLIO, ISNULL(P.NOMBRE,'') AS PROVEEDOR_GAS,  GV.SUBTOTAL, GV.IVA, ISNULL(GV.IMPORTE,0) AS NETO, ISNULL(GV.FACTURA,'') AS NUMFACTURA, 
                ISNULL(GV.FACTURA_CFDI,'') AS FAC_CFDI, ISNULL(P.CUENTA_CONTABLE,'') AS CTA_PROV, ISNULL(CAMPLIB1,'') AS LIB1, ISNULL(XML,'') AS FILE_XML
                ...
                WHERE (GV.ST_VALES = 'ACEPTADO' OR GV.ST_VALES = 'CONCILIADO') AND 
                FECHA_CFDI >= '" & FSQL(F1.Value) & "' AND FECHA_CFDI <= '" & FSQL(F2.Value) & "'
                ORDER BY GV.FOLIO"
            Using cmd As SqlCommand = cnSAE.CreateCommand
                cmd.CommandText = SQL
                Using dr As SqlDataReader = cmd.ExecuteReader
                    If dr.Read Then
                        FgF.AddItem("" & vbTab & "" & vbTab & dr("NUMFACTURA"))
                    End If
                End Using
            End Using
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        If Not Valida_Conexion() Then
            Return
        End If
        Dim SUBT As Decimal, IVA As Decimal, NETO As Decimal, nDoc As Integer, REFER As String, CVE_PROV As String
        Dim SUMA As Decimal = 0, S1 As Decimal, S2 As Decimal
            ...
            If MOV = "PAGA" Then
                For k = 1 To FgF.Rows.Count - 1
                    ...
                    If FgF(k, 1) Then
                        ...
                        SQL = "SELECT VC.CVE_COVC, VC.SUBTOTAL, VC.IVA, VC.NETO, VC.CVE_DOC AS NUMFACTURA, VC.FECHA_DOC AS FECHAFACTURA, 
                            P.NOMBRE AS PROVEEDOR_GAS, ISNULL(CAMPLIB1,'') AS LIB1, ISNULL(CAMPLIB2,'') AS LIB2, ISNULL(XML,'') AS FILE_XML
                            FROM GCCONCI_VALES_COMBUS VC 
                            LEFT JOIN PROV" & Empresa & " P ON P.CLAVE = VC.CVE_PROV
                            LEFT JOIN PROV_CLIB" & Empresa & " L ON L.CVE_PROV = P.CLAVE
                            WHERE 
                            ISNULL(VC.STATUS,'A') <> 'C' AND VC.CVE_DOC  = '" & REFER & "' AND 
                            VC.CVE_PROV = '" & CVE_PROV & "'"
                        Using cmd As SqlCommand = cnSAE.CreateCommand
                            cmd.CommandText = SQL
                            Using dr As SqlDataReader = cmd.ExecuteReader
                                If dr.Read Then
                                    Try
                                        CADENA = dr("PROVEEDOR_GAS") & "  F/" & dr("NUMFACTURA") & "  " & REFER
                                        ...
                                        Fg.AddItem("1" & vbTab & dr("LIB2") & vbTab & "0" & vbTab & CADENA & vbTab & S1 & vbTab & "" & vbTab &
                                                   dr("FILE_XML") & vbTab & "VC" & vbTab & dr("CVE_COVC"))
                                        If dr("LIB2").Trim.Length = 0 Then
                                            ...
                                            nDoc += 1
                                        End If
                                        Fg.AddItem(" " & vbTab & dr("LIB1") & vbTab & "0" & vbTab & CADENA & vbTab & S2 & vbTab & "" & vbTab &
                                                   dr("FILE_XML") & vbTab & "" & vbTab & "")
                                        If dr("LIB1").Trim.Length = 0 Then
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 1, NewStyle1)
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 2, NewStyle1)
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 3, NewStyle1)
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 4, NewStyle1)
                                            ...
                                            nDoc += 1
                                        End If
                                        Fg.AddItem("" & vbTab & "120-001-0000" & vbTab & "0" & vbTab & CADENA & vbTab & IVA & vbTab & "" & vbTab & "" & vbTab & "" & vbTab & "")
                                        Fg.AddItem("" & vbTab & "121-001-0000" & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & IVA & vbTab & "" & vbTab & "" & vbTab & "")
                                        Fg.AddItem("" & vbTab & "102-001-0001" & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & NETO & vbTab & "" & vbTab & "" & vbTab & "")
                                        SUMA += dr("NETO")
                                        nDoc += 1
                                    Catch ex As Exception
                                        Bitacora("30. " & ex.Message & vbNewLine & ex.StackTrace)
            ...
            Else
                For k = 1 To FgF.Rows.Count - 1
                    If FgF(k, 1) Then
                        ...
                        LEFT JOIN PROV" & Empresa & " P ON P.CLAVE = G.CVE_PROV
                        LEFT JOIN PROV_CLIB" & Empresa & " L ON L.CVE_PROV = P.CLAVE
                        WHERE GV.FACTURA = '" & FgF(k, 2) & "'"
                        Using cmd As SqlCommand = cnSAE.CreateCommand
                            cmd.CommandText = SQL
                            Using dr As SqlDataReader = cmd.ExecuteReader
                                If dr.Read Then
                                    Try
                                        CADENA = dr("PROVEEDOR_GAS") & "  F/" & dr("NUMFACTURA") & "  " & dr("FAC_CFDI")
                                        ...
                                        Fg.AddItem("1" & vbTab & dr("CTA_PROV") & vbTab & "0" & vbTab & CADENA & vbTab & SUBT + IVA & vbTab & "" & vbTab & dr("FILE_XML") & vbTab &
                                           "VH" & vbTab & dr("FOLIO"))
                                        If dr("CTA_PROV").Trim.Length = 0 Then
                                            ...
                                            nDoc += 1
                                        End If
                                        Fg.AddItem(" " & vbTab & dr("LIB1") & vbTab & "0" & vbTab & CADENA & vbTab & NETO - SUBT - IVA & vbTab & "" & vbTab & "" & vbTab & "" & vbTab & "")
                                        If dr("LIB1").Trim.Length = 0 Then
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 1, NewStyle1)
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 2, NewStyle1)
                                            Fg.SetCellStyle(Fg.Rows.Count - 1, 3, NewStyle1)
    ...
    Private Sub BarGenPoliza_Click(sender As Object, e As ClickEventArgs) Handles BarGenPoliza.Click
        ...
        Dim CADENA1 As String, XML As String, UUID_XML As String = ""
        ...
        Try
            ...
            Try
                If IsNumeric(Var46) Then
                    SQL = "UPDATE GCASIGNACION_VALES_UTIL SET POL_GEN = 'S' WHERE CVE_COVC = " & Var46
                    Using cmd As SqlCommand = cnSAE.CreateCommand
                        cmd.CommandText = SQL

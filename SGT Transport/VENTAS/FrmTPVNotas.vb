...
Public Class FrmTPVNotas
    ...
    Sub CARGAR_DATOS1()
            ...
            Case "D"
                ...
                If Ldocu.Tag = "Digital" Then
                    ...
                    If CVE_ART_NC.Trim.Length = 0 Then
                        MsgBox("Por favor capture la clave del Artículo para notas de credito en parametros-inventario")
                    End If
                End If
        End Select
        ...
        Try
            ...
            If TCVE_PEDI.Text <> "CARTA PORTE" Then
                ...
                Fg.Cols(26).Width = 0
            End If
            If TIPO_VENTA_LOCAL = "D" Or TIPO_VENTA_LOCAL = "F" Or TIPO_VENTA_LOCAL = "V" Or TIPO_VENTA_LOCAL = "P" Then
        ...
        Try
            ...
            If MODO_EDIT = "edit" Then
                If (TIPO_VENTA_LOCAL = "C" Or TIPO_VENTA_LOCAL = "R") And SE_MODIFICA Then
                    _myEditor = New MyEditorTPVta(Fg, LINEA_EN_VENTAS)
                    If Fg.Row > 0 Then
        ...
        End Try
    End Sub
    Private Sub FrmTPVNotas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub BarEnlaceDocumento_Click(sender As Object, e As EventArgs) Handles BarEnlaceDocumento.Click
        Try
            ...
            'TDOC = Var15
            'Select Case Var15
            'Case "P"   Lt1.Text = "Pedidos"
            'Case "C"   Lt1.Text = "Cotizaciones"
            'Case "F"   Lt1.Text = "Facturas"
            'End Select
        ...
        Try
            ...
            If TIPO_OTI = 0 Then
                SQL = "SELECT P.CVE_ART, CANT, I.DESCR, P.DESC1, PREC, IMPU1, IMPU2, IMPU3, IMPU4, IMP4APLICA, IMP1APLICA, COST, UNI_VENTA,
                    ...
                    ISNULL(ENLAZADO,'') AS ENLAZADO_DOC
            ...
            Else
                If EXIST_FIELD_SQL_SAE("PAR_FACT" & fTIPOC.ToUpper & "_CLIB" & Empresa, "CAMPLIB1") And
                    EXIST_FIELD_SQL_SAE("PAR_FACT" & fTIPOC.ToUpper & "_CLIB" & Empresa, "CAMPLIB2") And
                    EXIST_FIELD_SQL_SAE("PAR_FACT" & fTIPOC.ToUpper & "_CLIB" & Empresa, "CAMPLIB3") And
                    EXIST_FIELD_SQL_SAE("PAR_FACT" & fTIPOC.ToUpper & "_CLIB" & Empresa, "CAMPLIB4") And
                    EXIST_FIELD_SQL_SAE("PAR_FACT" & fTIPOC.ToUpper & "_CLIB" & Empresa, "CAMPLIB5") Then

                    SQL = "Select P.CVE_ART, CANT, I.DESCR, P.DESC1, PREC, IMPU1, IMPU2, IMPU3, IMPU4, IMP4APLICA, IMP1APLICA, COST, 
                        ...
                        DOC_SIG, ISNULL(ENLAZADO,'') AS ENLAZADO_DOC, ISNULL(CAMPLIB1, 0) AS LIB1, ISNULL(CAMPLIB2, '') AS LIB2, 
                        ISNULL(CAMPLIB3, '') AS LIB3, ISNULL(CAMPLIB4, '') AS LIB4, ISNULL(CAMPLIB5, '') AS LIB5, F.STATUS 
                        FROM PAR_FACT" & fTIPOC.ToUpper & Empresa & " P
                        LEFT JOIN FACT" & fTIPOC.ToUpper & Empresa & " F ON F.CVE_DOC = P.CVE_DOC
                        LEFT JOIN PAR_FACT" & fTIPOC.ToUpper & "_CLIB" & Empresa & " LP ON LP.CLAVE_DOC = P.CVE_DOC AND LP.NUM_PART = P.NUM_PAR
                        LEFT JOIN OBS_DOCF" & Empresa & " OD ON OD.CVE_OBS = F.CVE_OBS
                ...
                Else
                    SQL = "Select P.CVE_ART, CANT, i.DESCR, p.DESC1, PREC, IMPU1, IMPU2, IMPU3, IMPU4, IMP4APLICA, IMP1APLICA, COST, 
                        ...
                        ISNULL(CVE_PEDI,'') AS PEDI, ISNULL(INF.NOMBRE,'') AS INF_NOMBRE, ISNULL(F.CVE_OBS, 0) As CVE_OBS_DOC, 
                        ISNULL(P.CVE_OBS, 0) As CVE_OBS_PAR, ISNULL(OD.STR_OBS,'') AS DOC_OBS, ISNULL(OP.STR_OBS,'') AS PAR_OBS, TIP_DOC_SIG,
                        DOC_SIG, ISNULL(ENLAZADO,'') AS ENLAZADO_DOC, F.STATUS
                        From PAR_FACT" & fTIPOC.ToUpper & Empresa & " P
                        LEFT JOIN FACT" & fTIPOC.ToUpper & Empresa & " F ON F.CVE_DOC = P.CVE_DOC
                        LEFT JOIN OBS_DOCF" & Empresa & " OD ON OD.CVE_OBS = F.CVE_OBS
                        LEFT JOIN OBS_DOCF" & Empresa & " OP ON OP.CVE_OBS = P.CVE_OBS
            ...
            Do While dr.Read
                ...
                s = s & "" & vbTab '5
                s = s & dr("DESCR") & " (" & dr("TIPOELE") & ")" & vbTab '6
    ...
    Sub BUSCAR_CFDI(FFACTURA As String, FDOCUMENT As String, FDOCUMENT2 As String)
        ...
        ENTRA = True
    End Sub
    Private Sub FrmTPVNotas_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Try
            Me.Dispose()
    ...
    Private Sub TDesc_KeyDown(sender As Object, e As KeyEventArgs) Handles TDESC.KeyDown
        Try
            If e.KeyCode = 13 Or e.KeyCode = 9 Then
                If TIPO_VENTA_LOCAL = "F" Or TIPO_VENTA_LOCAL = "D" Or TIPO_VENTA_LOCAL = "V" Or TIPO_VENTA_LOCAL = "P" Then
                    CboPrecio.Focus()
                Else
                    TCVE_UNI.Focus()
                End If
    ...
    Private Function ExistenPartidas() As Boolean
        Try
            ...
            For k = 1 To Fg.Rows.Count - 1
                Try
                    If Fg(k, 1) > 0 And Fg(k, 4).ToString.Trim.Length > 0 Then
                        Siok = True
                        Exit For
                    End If
                Catch ex As Exception
    ...
    Sub GUARDAR()
        ...
        Dim Bloq As String, FORMAENVIO As String, DES_FIN_PORC As Decimal, DES_TOT_PORC As Decimal, COM_TOT_PORC As Decimal, METODODEPAGO As String
        'variables partidas
        ...
        Dim DESDE_INVE As String, MOV_ENLAZADO As Integer, USO_CFDI As String, FORMADEPAGOSAT As String, UNI_MED As String
        ...
        CVE_DOC = "" : CVE_CPTO = 51
        UNI_MED = "" : TIP_CAM = 1 : FACTOR_CON = 1 : REG_SERIE = 0 : E_LTPD = 0 : MOV_ENLAZADO = 0 : DAT_MOSTR = 0
        ...
        For k = 1 To Fg.Rows.Count - 1
            ...
            If CANT > 0 And CVE_ART.Trim.Length > 0 Then
                ...
                Try
                    ...
                    Try
                        'SQL = "IF NOT EXISTS (SELECT CLAVE_DOC FROM PAR_FACT" & TIPO_VENTA_LOCAL & "_CLIB" & Empresa & "
                        'WHERE CLAVE_DOC = @CLAVE_DOC And NUM_PART = @NUM_PART)
                        'INSERT INTO PAR_FACT" & TIPO_VENTA_LOCAL & "_CLIB" & Empresa & " (CLAVE_DOC, NUM_PART) VALUES(@CLAVE_DOC, @NUM_PART)"
                        'Using cmd3 As SqlCommand = cnSAE.CreateCommand
                        'cmd3.Parameters.Clear()
                        'cmd3.Parameters.Add("@CLAVE_DOC", SqlDbType.VarChar).Value = CVE_DOC
                        'cmd3.Parameters.Add("@NUM_PART", SqlDbType.Int).Value = NUM_PAR
                        'cmd3.CommandText = SQL
                        'returnValue = cmd3.ExecuteNonQuery().ToString
                        'If returnValue IsNot Nothing Then
                        'If returnValue = "1" Then
    ...
    Private Sub Fg_BeforeEdit(sender As Object, e As RowColEventArgs) Handles Fg.BeforeEdit
        ...
        Try
            If ENTRA Then
                If Fg.Row > 0 Then
                    If Fg.Col = 1 Or Fg.Col = 2 Or Fg.Col = 4 Or Fg.Col = 5 Or Fg.Col = 8 Or Fg.Col = 13 Then

                    Else
                        If e.Col = 3 Then
                            'Fg.Cols(3).Editor = countryCombo
                        Else
                            e.Cancel = True
                        End If
                    End If
    ...
    Private Sub Fg_Click(sender As Object, e As EventArgs) Handles Fg.Click
        Try        'SE_MODIFICA = false
            ...
            If ENTRA Then
                ...
                Else
                    If Fg.Col = 3 Then
                        If Fg.MouseRow < Fg.Rows.Fixed Then
                            Return
                        End If
                    End If
                End If
    ...
    Private Sub Fg_StartEdit(sender As Object, e As RowColEventArgs) Handles Fg.StartEdit
        ...
        Try
            If ENTRA Then
                If Fg.Col = 1 Or Fg.Col = 2 Or Fg.Col = 3 Or Fg.Col = 4 Or Fg.Col = 5 Or Fg.Col = 8 Or Fg.Col = 13 Then
                    'e.Cancel = True
                Else
                    'e.Cancel = True
                End If
                    ...
                    Case 5
                        Try
                            ...
                            If Var4.Trim.Length > 0 Then
                                ...
                                If TIPO_VENTA = "F" Or TIPO_VENTA = "V" Then
                                    If Fg(Fg.Row, 1) > EXISTECI Then
    ...
    Sub LLENAR_GRID_ARTICULO(FCVE_ART As String)
        Try
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            Dim ULT_COSTO As Single, PRECIO As Single, CVE_ALM_TPV As Integer = 1
            ...
            If MULTIALMACEN = 0 Then
                SQL = "SELECT DESCR, ULT_COSTO, UNI_MED, IMPUESTO1, IMPUESTO2, IMPUESTO3, IMPUESTO4, I.CVE_ESQIMPU,
                    ...
                    ISNULL(EXIST,0) AS EXIS
                    FROM INVE" & Empresa & " I
                    LEFT JOIN CVES_ALTER" & Empresa & " A ON A.CVE_ART = I.CVE_ART
                    LEFT JOIN IMPU" & Empresa & " P ON P.CVE_ESQIMPU = I.CVE_ESQIMPU
                    WHERE (I.CVE_ART = '" & FCVE_ART & "' OR ISNULL(A.CVE_ALTER,'') = '" & FCVE_ART & "') AND 
                    ISNULL(A.CVE_ALTER,'') <> ''"
            Else
                SQL = "SELECT DESCR, ULT_COSTO, UNI_MED, IMPUESTO1, IMPUESTO2, IMPUESTO3, IMPUESTO4, I.CVE_ESQIMPU,
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 1),0) AS P1,
                    ...
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 13),0) AS P13,
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 14),0) AS P14,
                    ISNULL(M.EXIST,0) AS EXIS
                    FROM MULT" & Empresa & " M
                    LEFT JOIN INVE" & Empresa & " I ON I.CVE_ART = M.CVE_ART
                    LEFT JOIN CVES_ALTER" & Empresa & " A ON A.CVE_ART = I.CVE_ART
                    LEFT JOIN IMPU" & Empresa & " P ON P.CVE_ESQIMPU = I.CVE_ESQIMPU
                    WHERE M.CVE_ALM = " & CVE_ALM_TPV & " AND ISNULL(A.CVE_ALTER,'') <> '' AND 
            ...
            If dr.Read Then
                ...
                End If
                Fg(Fg.Row, 14) = Fg(Fg.Row, 1) * Fg(Fg.Row, 13)
    ...
    Private Sub BarLimpiar_Click(sender As Object, e As EventArgs) Handles BarLimpiar.Click
        Try
            'INSERT_UPDATE_OBS_FAC_CLIB(LtCVE_DOC.Text, "C", Obser)
            LIMPIAR()
        Catch ex As Exception
    ...
    Sub CANCELAR_DOCUMENTO()
        ...
        Try
            ...
            If TIPO_VENTA_LOCAL = "V" Or TIPO_VENTA_LOCAL = "R" Then
                Try
    ...
    Private Sub BarKardex_Click(sender As Object, e As EventArgs) Handles BarKardex.Click
        Try
            If Fg.Row > 0 Then
                ...
                If Var4.Trim.Length = 0 Then
                    MsgBox("Por favor seleccione una partida que contenga la clave del Artículo a consultar el kardex")
                    Return
                End If
                Var5 = ""
                Try
    ...
    Private Sub CargarRegimenesFiscales()
        Dim lista As List(Of cRegimenFiscal) = New List(Of cRegimenFiscal) From {
...
Public Class MyEditorTPVta
    ...
    Private EXISTENCIA As Decimal = 0
    Private ENTRA_MYEDIT As String = True, LIN_EN_VTA As String

    'constructor: attach to owner grid
    'Constructor: adjuntar a la cuadrícula del propietario
    Public Sub New(ByVal owner As C1FlexGrid, FLIN_EN_VTA As String)
        MyBase.New()
        Visible = False
        AutoSize = False
    ...
    Public Sub StartEditing(ByVal row As Integer, ByVal col As Integer, Optional ByVal keyRec As Integer = 0)
        ...
        Dim PRECIO As Single, SiGue As Boolean = False
        ...
        If col = 1 And keyRec = 99 Then
            ...
            'End If
        End If
        If col = 2 Then 'ALMACEN
            If Not IsNothing(_owner(_row, 2)) Then
                If _owner(_row, 2) = "" Or _owner(_row, 2) = "0" Then
                    _owner(_row, 2) = "1"
                End If
            Else
                _owner(_row, 2) = "1"
        ...
        If col = 1 And keyRec = 133 Then
            ...
            Return
        End If
        If col = 1 Or col = 2 Or col = 4 Or col = 8 Or col = 13 Then
            If col <> 1 Then
                If Not FgEdit Then
                    _owner.Col = 1

                    FrmTPVNotas.tMagico2.Focus()
                    MyBase.Visible = True
                    '_owner.Select()
    ...
    Public Sub EnterCell(ByVal row As Integer, ByVal col As Integer)
        ...
        If col = 1 Or col = 2 Or col = 4 Or col = 5 Or col = 13 Then
            MyBase.Visible = True
        End If
    End Sub
    ...
    Protected Overrides Sub OnLeave(ByVal e As System.EventArgs)
        ...
        Try
            If _col = 1 Then 'cantidad
                Try
                    If Not IsNothing(MyBase.Text) Then
                        If IsNumeric(MyBase.Text) Then
                            Var13 = "SOLOTOTAL"
                            FrmTPVNotas.CALCULAR_IMPORTES(_row, CDec(MyBase.Text))
                        End If
                    End If
                Catch ex As Exception
                ...
                End Try
            End If
            If _col = 4 And ENTRA_MYEDIT Then
                Dim Descr As String
                ...
                If MyBase.Text.Trim.Length > 0 Then
                    ...
                    If Descr.TrimEnd.Trim.Length = 0 Or Descr = "N" Then
                        If MULTIALMACEN = 0 Then
                            MsgBox("Artículo inexistente")
                        Else
                            MsgBox("Artículo inexistente en el alamcen seleccionado")
                        End If
                        FrmTPVNotas.tMagico2.Focus()
                    ...
                    Else
                        ...
                        If EXISTENCIA = -9999998989 Then
                            If MULTIALMACEN = 0 Then
                                MsgBox("Artículo inexistente")
                            Else
                                MsgBox("Artículo inexistente en el almacén seleccionado")
                            End If
                            FrmTPVNotas.tMagico2.Focus()
                            _owner.Select(_row, _col)
                            _owner(_row, _col) = ""
                        ...
                        Else
                            If TIPO_VENTA = "F" Or TIPO_VENTA = "V" Then
                                If _owner(_row, 1) > EXISTENCIA Then
                                    MsgBox("Existencia insuficiente")
                                    FrmTPVNotas.tMagico2.Focus()
                                    _owner.Select(_row, 1)
                                    _owner(_row, 1) = 0
                        ...
                        If Not String.IsNullOrEmpty(_owner(_row, _col)) Then
                            If IsNumeric(_owner(_row, 1)) Then
                                If IsNumeric(MyBase.Text) Then
                                    Try
                                        Dim C1 As Decimal
                                        C1 = (_owner(_row, 1) * _owner(_row, 13)) - (_owner(_row, 1) * _owner(_row, 13) * MyBase.Text / 100)
                                        _owner(_row, 14) = C1

                                    Catch ex As Exception
                                        _owner(_row, 14) = _owner(_row, 1) * _owner(_row, 13)
                                    End Try
                                    Var13 = "SOLOTOTAL"
                                    FrmTPVNotas.CALCULAR_IMPORTES()
                                End If
                            End If
                        End If
                    End If
                        ...
                        If Not String.IsNullOrEmpty(_owner(_row, _col)) Then
                            If IsNumeric(_owner(_row, 1)) Then
                                If IsNumeric(MyBase.Text) Then
                                    Try
                                        Dim C1 As Decimal
                                        C1 = (_owner(_row, 1) * MyBase.Text) - (_owner(_row, 1) * MyBase.Text * _owner(_row, 8) / 100)
                                        _owner(_row, 14) = C1

                                    Catch ex As Exception
                                        _owner(_row, 14) = _owner(_row, 1) * MyBase.Text
                                    End Try
                                    Var13 = "SOLOTOTAL"
                                    FrmTPVNotas.CALCULAR_IMPORTES()
                                End If
        ...
        Try
            Select Case e.KeyChar
                Case Chr(13), Chr(9)
                    ...
                    Select Case col
                        Case 1
                            If _owner.Cols(2).IsVisible Then
                        ...
                        Case 13
                            Try
                                If OBSER_X_PARTIDA = 1 Then
                                    ...
                                    Var6 = "PAR_FACT_CLIB"
                                    Var7 = "PAR_FACT" & TIPO_VENTA & "_CLIB" & Empresa
                                    Var20 = FrmTPVNotas.Fg.Row

                                    If DOC_NEW Then
                                        Var8 = ""
                                    Else
                                        Var8 = FrmTPVNotas.LtCVE_DOC.Text
                                    End If
                            ...
                            Try
                                ...
                                If HayErr Then
                                    ...
                                    End If
                                Else
                                    If _owner(row, 1) > 0 And _owner(row, 4).ToString.Trim.Length > 0 Then
    ...
    Sub LLENAR_GRID_ARTICULO(FCVE_ART As String)
        Try
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            Dim ULT_COSTO As Single, PRECIO As Decimal = 0, CVE_ALM_MY As Integer = 1
            ...
            Try
                If MULTIALMACEN = 1 Then
                    CADENA_ALM = FrmTPVNotas.CboAlmacen.SelectedIndex
                    If FrmTPVNotas.CboAlmacen.SelectedIndex > -1 Then
                        If Not IsNothing(_owner(_owner.Row, 3)) Then
                            Try
                                If _owner(_owner.Row, 3).ToString.Trim.Length > 0 Then
                                    CVE_ALM_MY = _owner(_owner.Row, 3)
                                Else
                                    CVE_ALM_MY = FrmTPVNotas.CboAlmacen.Items(FrmTPVNotas.CboAlmacen.SelectedIndex).ToString.Substring(0, 2)
                                End If
                            Catch ex As Exception
                                BITACORATPV("3080. " & ex.Message & vbNewLine & ex.StackTrace)
                            End Try
                        Else
                            Try
                                CADENA_ALM = FrmTPVNotas.CboAlmacen.Text.Substring(0, 2)
                                CVE_ALM_MY = FrmTPVNotas.CboAlmacen.Items(FrmTPVNotas.CboAlmacen.SelectedIndex).ToString.Substring(0, 2)
                            Catch ex As Exception
                                BITACORATPV("3080. " & ex.Message & vbNewLine & ex.StackTrace)
                            End Try
            ...
            If MULTIALMACEN = 0 Then
                SQL = "SELECT I.CVE_ART, DESCR, ULT_COSTO, UNI_MED, IMPUESTO1, IMPUESTO2, IMPUESTO3, IMPUESTO4, I.CVE_ESQIMPU,
                    ...
                    ISNULL(A.CVE_ALTER,'') <> ''"
            Else
                SQL = "SELECT M.CVE_ART, DESCR, ULT_COSTO, UNI_MED, IMPUESTO1, IMPUESTO2, IMPUESTO3, IMPUESTO4, I.CVE_ESQIMPU,
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 1),0) AS P1,
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 2),0) AS P2,
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 3),0) AS P3,
                    ISNULL((SELECT PRECIO FROM PRECIO_X_PROD" & Empresa & " P WHERE P.CVE_ART = I.CVE_ART AND CVE_PRECIO = 4),0) AS P4,
                    ...
                    LEFT JOIN CVES_ALTER" & Empresa & " A ON A.CVE_ART = I.CVE_ART
                    LEFT JOIN IMPU" & Empresa & " P ON P.CVE_ESQIMPU = I.CVE_ESQIMPU
                    WHERE M.CVE_ALM = " & CVE_ALM_MY & " AND ISNULL(A.CVE_ALTER,'') <> '' AND 
                    (M.CVE_ART = '" & FCVE_ART & "' OR ISNULL(A.CVE_ALTER,'') = '" & FCVE_ART & "')"
            End If
            cmd.CommandText = SQL
            dr = cmd.ExecuteReader
            If dr.Read Then
                ...
                _owner(_row, 8) = FrmTPVNotas.TDESC.Text
                ...
                If _owner(_row, 15) <> FCVE_ART Then
                    'PRECIO = FrmTPVNotas.cboPrecio.Items(FrmTPVNotas.cboPrecio.SelectedIndex).ToString.Substring(0, 2)
                    Select Case FrmTPVNotas.CboPrecio.Items(FrmTPVNotas.CboPrecio.SelectedIndex).ToString.Substring(0, 2)
                        Case "01"
                            PRECIO = dr("P1")
                        Case "02"
                            PRECIO = dr("P2")
                ...
                Try
                    Dim C1 As Decimal
                    '       CANTIDAD                         CANT                     DESC       
                    C1 = (_owner(_row, 1) * PRECIO) - (_owner(_row, 1) * PRECIO * _owner(_row, 8) / 100)
                    _owner(_row, 14) = C1
                Catch ex As Exception
                    _owner(_row, 14) = _owner(_row, 1) * PRECIO
                End Try
                _owner(_row, 15) = FCVE_ART
                Var13 = "SOLOTOTAL"
                FrmTPVNotas.CALCULAR_IMPORTES()
            Else
                EXISTENCIA = -9999998989
            End If
            dr.Close()
        Catch ex As Exception

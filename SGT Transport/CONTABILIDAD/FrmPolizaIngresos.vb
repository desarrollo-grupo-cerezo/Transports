...
Public Class FrmPolizaIngresos

    Private EmisorRfcCFD As String
    Private ReceptorRfcCFD As String
    ...
    Private Sub FrmPolizaIngresos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            FgF.Height = H1
            FgF.Styles.EmptyArea.BackColor = Color.Honeydew
            ...
            'CADENA1 = "<ROWCDSPartidasUUID NUMREG=""" & NUM_PARX & """ UUIDTIMBRE=""" & UUID_XML & """ MONTO=""" & TotalCFD & """ SERIE=""" & SerieCFD & 
            '""" FOLIO=""" & FolioCFD & """ RFCEMISOR=""" & EmisorRfcCFD & """ RFCRECEPTOR=""TEM820612AF7"" FECHAUUID=""" &
            'FechaEmision & """ TIPOCOMPROBANTE=""1""/>"

            Fg(0, 7) = "UUID"
            Dim cc3 As Column = Fg.Cols(7)
            cc3.DataType = GetType(String)

            Fg(0, 8) = "TOTALCFD"
            Dim cc4 As Column = Fg.Cols(8)
            cc4.DataType = GetType(Decimal)
            Fg.Cols(4).Format = "###,###,##0.00"

            Fg(0, 9) = "SERIECFD"
            Dim cc5 As Column = Fg.Cols(9)
            cc5.DataType = GetType(String)

            Fg(0, 10) = "FOLIOCFD"
            Dim cc6 As Column = Fg.Cols(10)
            cc6.DataType = GetType(String)

            Fg(0, 11) = "EMISOR RFC"
            Dim cc7 As Column = Fg.Cols(11)
            cc7.DataType = GetType(String)

            Fg(0, 12) = "RECEPTOR RFC"
            Dim cc8 As Column = Fg.Cols(12)
            cc8.DataType = GetType(String)

            Fg(0, 13) = "FECHA EMISION"
            Dim cc9 As Column = Fg.Cols(13)
            cc9.DataType = GetType(String)
            ...
            tPOLIZA.Text = "Poliza ingresos fecha " & DateTime.Now.ToShortDateString
        Catch ex As Exception
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaIngresos_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Poliza ingreso")
    End Sub
    ...
    Private Sub BtnFiltrar_Click(sender As Object, e As EventArgs) Handles BtnFiltrar.Click
        Try
            ...
            Using cmd2 As SqlCommand = cnSAE.CreateCommand
                SQL = "SELECT D.REFER, M.NO_FACTURA, CLAVE, NOMBRE, CVE_DOC_COMPPAGO
                ...
                SQL = "SELECT ISNULL(D.DOCTO,'') AS DOCT, SUM(D.IMPORTE) AS IMPORTE_DOCTO, MAX(CLAVE) AS CLIENTE, MAX(NOMBRE) AS NOMB, 
                    MAX(CVE_DOC_COMPPAGO) AS DOC_COMP
                    FROM CUEN_DET" & Empresa & " D 
                    LEFT JOIN CLIE" & Empresa & " P ON P.CLAVE = D.CVE_CLIE
                    WHERE FECHA_APLI >= '" & FSQL(F1.Value) & "' AND FECHA_APLI <= '" & FSQL(F2.Value) & "'
                    GROUP BY DOCTO ORDER BY MAX(CVE_DOC_COMPPAGO) DESC"

                cmd2.CommandText = SQL
                Using dr2 As SqlDataReader = cmd2.ExecuteReader
                    While dr2.Read
                        FgF.AddItem("" & vbTab & "" & vbTab & dr2("DOCT") & vbTab & dr2("IMPORTE_DOCTO") & vbTab &
                                    "(" & dr2("CLIENTE") & ") " & dr2("NOMB") & vbTab & dr2("DOC_COMP"))
                    End While
                End Using 'PAFA_DET
            End Using
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        If Not Valida_Conexion() Then
            Return
        End If
        Dim SUBT As Decimal = 0, IVA As Decimal, IMPORTE As Decimal, nDoc As Integer, DOCTO As String
        Dim SUMA As Decimal = 0, CTA_PROV As String = "", CTA_PROV_LIB As String = "", z As Integer = 0, XML As String
        Dim s1 As String = "", s2 As String = "", s3 As String = "", s4 As String = "", SIGUE As Boolean = True, IMPORTE_DOCTO As Decimal
        Dim UUID_XML As String, CTA_CONT As String
        ...
        Try
            ...
            For k = 1 To FgF.Rows.Count - 1
                If FgF(k, 1) Then
                    z += 1
                    DOCTO = FgF(k, 2)
                    IMPORTE_DOCTO = FgF(k, 3)

                    SQL = "SELECT ISNULL(D.DOCTO,'') AS DOCT, SUM(D.IMPORTE) AS IMPORTE_DOCTO, MAX(CLAVE) AS CLIENTE, MAX(NOMBRE) AS NOMB, 
                        MAX(CVE_DOC_COMPPAGO) AS DOC_COMP
                        FROM CUEN_DET" & Empresa & " D 
                        LEFT JOIN CLIE" & Empresa & " P ON P.CLAVE = D.CVE_CLIE
                        WHERE FECHA_APLI >= '" & FSQL(F1.Value) & "' AND FECHA_APLI <= '" & FSQL(F2.Value) & "' AND 
                        DOCTO = '" & DOCTO & "'
                        GROUP BY DOCTO"
                    ...
                    Using cmd2 As SqlCommand = cnSAE.CreateCommand
                        ...
                        Using dr2 As SqlDataReader = cmd2.ExecuteReader
                            While dr2.Read
                                ...
                                Fg.AddItem("1" & vbTab & "102-001-0001" & vbTab & "0" & vbTab & DOCTO.ToString.Trim &
                                   " Cobranza del dia " & F1.Value & " al " & F2.Value & vbTab &
                                   IMPORTE_DOCTO & vbTab & "" & vbTab & "")

                                z = Fg.Rows.Count - 1

                                SQL = "SELECT D.REFER, D.CVE_CLIE, NOMBRE, D.IMPORTE, CFDI.SUBTOTAL, CFDI.RETENCION, CFDI.IVA, 
                                    CFDI.IMPORTE AS IMPORTE_CFDI, ISNULL(CUENTA_CONTABLE,'') AS CTA_CLIE, L.CAMPLIB2 AS LIB2, 
                                    ISNULL(XML,'') AS XML_TRANS, ISNULL(XML_DOC,'') AS XML_SAE
                                    FROM CUEN_DET" & Empresa & " D 
                                    LEFT JOIN CLIE" & Empresa & " P ON P.CLAVE = D.CVE_CLIE
                                    LEFT JOIN CLIE_CLIB" & Empresa & " L ON L.CVE_CLIE = D.CVE_CLIE
                                    LEFT JOIN CFDI ON CFDI.FACTURA = D.REFER
                                    LEFT JOIN CFDI" & Empresa & " FD ON FD.CVE_DOC = D.REFER AND TIPO_DOC = 'F'
                                    WHERE FECHA_APLI >= '" & FSQL(F1.Value) & "' AND FECHA_APLI <= '" & FSQL(F2.Value) & "' AND 
                                    DOCTO = '" & DOCTO & "' ORDER BY D.REFER"
                                Using cmd As SqlCommand = cnSAE.CreateCommand
                                    cmd.CommandText = SQL
                                    Using dr As SqlDataReader = cmd.ExecuteReader
                                        Do While dr.Read
                                            Try
                                                If dr("XML_TRANS").ToString.Trim.Length > 0 Then
                                                    XML = dr("XML_TRANS")
                                                Else
                                                    XML = dr("XML_SAE")
                                                End If
                                                ...
                                                If XML.Trim.Length > 0 Then
                                                    ...
                                                    Else
                                                        IVA = (dr("IMPORTE") / 1.12) * 0.16
                                                    End If

                                                    If RETcfd > 0 Then
                                                        CTA_CONT = dr.ReadNullAsEmptyString("CTA_CLIE")
                                                    Else
                                                        CTA_CONT = dr.ReadNullAsEmptyString("LIB2")
                                                    End If
                                                    'CADENA1 = "<ROWCDSPartidasUUID NUMREG=""" & NUM_PARX & """ UUIDTIMBRE=""" & UUID_XML & """ MONTO=""" & TotalCFD & """ SERIE=""" & SerieCFD & 
                                                    '""" FOLIO=""" & FolioCFD & """ RFCEMISOR=""" & EmisorRfcCFD & """ RFCRECEPTOR=""TEM820612AF7"" FECHAUUID=""" &
                                                    'FechaEmision & """ TIPOCOMPROBANTE=""1""/>"

                                                    Fg.AddItem("2" & vbTab & "222-001-0001" & vbTab & "0" & vbTab & CADENA & vbTab & IVA & vbTab & "" & vbTab & "")
                                                    Fg.AddItem("" & vbTab & "221-001-0001" & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & IVA & vbTab & "")
                                                    Fg.AddItem("" & vbTab & CTA_CONT & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & IMPORTE & vbTab &
                                                       XML & vbTab & UUID_XML & vbTab & IMPORTE & vbTab & SerieCFD & vbTab & FolioCFD & vbTab &
                                                       EmisorRfcCFD & vbTab & ReceptorRfcCFD & vbTab & FechaEmision)

                                                    If dr("CTA_CLIE").Trim.Length = 0 Then
                                                        ...
                                                        nDoc = nDoc + 1
                                                    End If
                                                    SUMA = SUMA + dr("IMPORTE")
                                                    nDoc = nDoc + 1
                                                Else
                                                    UUID_XML = "" 'OBTENER_UUID_XML(XML)
                                                    CADENA = dr.ReadNullAsEmptyString("NOMBRE") & "  " & dr.ReadNullAsEmptyString("REFER")
                                                    IMPORTE = dr("IMPORTE")
                                                    IVA = (dr("IMPORTE") / 1.12) * 0.16
                                                    If RETcfd > 0 Then
                                                        CTA_CONT = dr.ReadNullAsEmptyString("CTA_CLIE")
                                                    ...
                                                    Fg.AddItem("" & vbTab & "221-001-0001" & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & IVA & vbTab & "")
                                                    Fg.AddItem("" & vbTab & CTA_CONT & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & IMPORTE & vbTab &
                                                       XML & vbTab & UUID_XML & vbTab & IMPORTE & vbTab & "" & vbTab & "" & vbTab &
                                                       "" & vbTab & "" & vbTab & "")
                                                    BACKUPTXT("DOCUMENTOS SIN XML", dr("REFER"))
    ...
    Private Sub BarExcel_Click(sender As Object, e As ClickEventArgs) Handles BarExcel.Click
        Try
            EXPORTAR_EXCEL_TRANSPORT(Fg, "Poliza ingresos")
        Catch ex As Exception
            MsgBox("12. " & ex.Message & vbNewLine & ex.StackTrace)
            Bitacora("12. " & ex.Message & vbNewLine & ex.StackTrace)
        End Try
    ...
    Private Sub BarGenPoliza_Click(sender As Object, e As ClickEventArgs) Handles BarGenPoliza.Click
        ...
        Try
            ...
            s = s & "<ROW VersionCOI=""810"" TipoPoliz=""Ig"" DiaPoliz=""30"" ConcepPoliz=""" & tPOLIZA.Text & """><Partidas>"
            'BACKUPTXT("XML", s)
        Catch ex As Exception
            ...
            For k = 1 To Fg.Rows.Count - 1
                ...
                If CUENTA.Trim.Length > 0 Then
                    ...
                    If XML.Trim.Length > 0 Then
                        ...
                        'Fg.AddItem("" & vbTab & CTA_CONT & vbTab & "0" & vbTab & CADENA & vbTab & "" & vbTab & IMPORTE & vbTab &
                        '     6                 7                  8                9                  10
                        'dr("XML") & vbTab & UUID_XML & vbTab & IMPORTE & vbTab & SerieCFD & vbTab & FolioCFD & vbTab &
                        '     11                    12 
                        'EmisorRfcCFD & vbTab & ReceptorRfcCFD)
                        UUID_XML = Fg(k, 7)
                        TotalCFD = Fg(k, 8)
                        SerieCFD = Fg(k, 9)
                        FolioCFD = Fg(k, 10)
                        EmisorRfcCFD = Fg(k, 11)
                        ReceptorRfcCFD = Fg(k, 12)
                        FechaEmision = Fg(k, 13)
                        ...
                        If UUID_XML.Trim.Length > 0 Then
                            CADENA1 = "<ROWCDSPartidasUUID NUMREG=""" & NUM_PARX & """ UUIDTIMBRE=""" & UUID_XML & """ MONTO=""" & TotalCFD & """ SERIE=""" & SerieCFD & """ FOLIO=""" & FolioCFD & """ RFCEMISOR=""" & EmisorRfcCFD & """ RFCRECEPTOR=""" & ReceptorRfcCFD & """ FECHAUUID=""" &
                                FechaEmision & """ TIPOCOMPROBANTE=""1""/>"
                        Else
                            CADENA1 = ""
                        End If
    ...
    Private Function OBTENER_UUID_XML(fXML As String) As String
        ...
        XML = OBTENER_RUTA_XML_TRANS_PDF() & "\TEMPOLXML.xml"
    ...
    Private Sub F1_DropDownClosed(sender As Object, e As DropDownClosedEventArgs) Handles F1.DropDownClosed
        Try
            ...
            tPOLIZA.Text = "Poliza ingresos fecha " & F1.Value

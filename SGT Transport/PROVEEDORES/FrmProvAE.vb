...
Public Class FrmProvAE
    ...
    Private Sub BarGrabar_Click(sender As Object, e As EventArgs) Handles BarGrabar.Click
        ...
        SQL = "UPDATE PROV" & Empresa & " SET CLAVE = @CLAVE, NOMBRE = @NOMBRE, RFC = @RFC, CALLE = @CALLE, NUMINT = @NUMINT, " &
            ...
            "TELEFONO = @TELEFONO, CLASIFIC = @CLASIFIC, FAX = @FAX, PAG_WEB = @PAG_WEB, CURP = @CURP, " &
            "CVE_ZONA = @CVE_ZONA, CON_CREDITO = @CON_CREDITO, DIASCRED = @DIASCRED, LIMCRED = @LIMCRED, CVE_BITA = @CVE_BITA, " &
            "ULT_PAGOD = @ULT_PAGOD, ULT_PAGOM = @ULT_PAGOM, ULT_PAGOF = @ULT_PAGOF, ULT_COMPD = @ULT_COMPD, ULT_COMPM = @ULT_COMPM, " &
            "ULT_COMPF = @ULT_COMPF, SALDO = @SALDO, VENTAS = @VENTAS, DESCUENTO = @DESCUENTO, TIP_TERCERO = @TIP_TERCERO, " &
            "TIP_OPERA = @TIP_OPERA, CVE_OBS = @CVE_OBS, CUENTA_CONTABLE = @CUENTA_CONTABLE, FORMA_PAGO = @FORMA_PAGO, " &
            "BENEFICIARIO = @BENEFICIARIO, TITULAR_CUENTA = @TITULAR_CUENTA, BANCO = @BANCO, SUCURSAL_BANCO = @SUCURSAL_BANCO, " &
            "CUENTA_BANCO = @CUENTA_BANCO, CLABE = @CLABE, DESC_OTROS = @DESC_OTROS, IMPRIR = @IMPRIR, MAIL = @MAIL, ENVIOSILEN = @ENVIOSILEN, " &
            "EMAILPRED = @EMAILPRED, MODELO = @MODELO  " &
            "WHERE CLAVE = @CLAVE " &
            "IF @@ROWCOUNT = 0 " &
            "INSERT INTO PROV" & Empresa & " (CLAVE, STATUS, NOMBRE, RFC, CALLE, NUMINT, NUMEXT, CRUZAMIENTOS, CRUZAMIENTOS2, COLONIA, CODIGO, LOCALIDAD, " &
            "MUNICIPIO, ESTADO, CVE_PAIS, NACIONALIDAD, TELEFONO, CLASIFIC, FAX, PAG_WEB, CURP, CVE_ZONA, CON_CREDITO, DIASCRED, LIMCRED, " &
            "CVE_BITA, ULT_PAGOD, ULT_PAGOM, ULT_PAGOF, ULT_COMPD, ULT_COMPM, ULT_COMPF, SALDO, VENTAS, DESCUENTO, TIP_TERCERO, TIP_OPERA, " &
            "CVE_OBS, CUENTA_CONTABLE, FORMA_PAGO, BENEFICIARIO, TITULAR_CUENTA, BANCO, SUCURSAL_BANCO, " &
            "CUENTA_BANCO, CLABE, DESC_OTROS, IMPRIR, MAIL, ENVIOSILEN, EMAILPRED, MODELO) VALUES (@CLAVE, 'A', @NOMBRE, @RFC, " &
            "@CALLE, @NUMINT, @NUMEXT, @CRUZAMIENTOS, @CRUZAMIENTOS2, @COLONIA, @CODIGO, @LOCALIDAD, @MUNICIPIO, @ESTADO, @CVE_PAIS, " &
            "@NACIONALIDAD, @TELEFONO, @CLASIFIC, @FAX, @PAG_WEB, @CURP, @CVE_ZONA, @CON_CREDITO, @DIASCRED, @LIMCRED, @CVE_BITA, " &
            "@ULT_PAGOD, @ULT_PAGOM, @ULT_PAGOF, @ULT_COMPD, @ULT_COMPM, @ULT_COMPF, @SALDO, @VENTAS, @DESCUENTO, @TIP_TERCERO, @TIP_OPERA, " &
            "@CVE_OBS, @CUENTA_CONTABLE, @FORMA_PAGO, @BENEFICIARIO, @TITULAR_CUENTA, @BANCO, @SUCURSAL_BANCO, @CUENTA_BANCO, " &
            "@CLABE, @DESC_OTROS, @IMPRIR, @MAIL, @ENVIOSILEN, @EMAILPRED, @MODELO)"
        cmd.CommandText = SQL
        Try

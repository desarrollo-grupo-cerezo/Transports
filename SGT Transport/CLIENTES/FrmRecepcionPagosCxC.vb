...
Public Class frmRecepcionPagosCxC
    Private ENTRA As Boolean
    Private SIGUE As Boolean = True
    Private _myEditor As MyEditorCxC

    Private Sub frmRecepcionPagosCxC_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        '20 FEB 20
        ...
        Try
            ...
            _myEditor = New MyEditorCxC(Fg, 0)
    ...
    Private Sub BarGrabar_Click(sender As Object, e As EventArgs) Handles barGrabar.Click
        ...
        Try
            ...
            Next
            ENTRA = True
            'UPDATE_DATOS_CLIE(SUMAPAGOS * -1, tREFER.Text, TCLIENTE.Text, " - ")
            FORMAPAGO = "S"
            LIMPIAR()
    ...
    Sub CUEN_DET_COM(fCVE_DOC As String, fIMPORTE As Decimal, fNUM_CPTO As Integer, fDOCTO As String)
        ...
        Try
            ...
            SQL = "INSERT INTO CUEN_DET" & Empresa & " (CVE_CLIE, REFER, ID_MOV, NUM_CPTO, NUM_CARGO, CVE_OBS, NO_FACTURA, DOCTO, IMPORTE,
                  ...
                  "','" & FSQL(F1.Value) & "','" & FSQL(F1.Value) & "','" & AFEC_COI & "','" & NUM_MONED & "','" &
                  TCAMBIO & "','" & Math.Round(IMPORTE, 6) & "',GETDATE(),'" & CTLPOL & "','" & CVE_FOLIO & "','" & TIPO_MOV & "','" &
                  SIGNO & "','" & CVE_AUT & "','" & Usuario2 & "','" & REF_SIST & "',
                  ISNULL((SELECT MAX(NO_PARTIDA) + 1 FROM CUEN_DET" & Empresa & " WHERE REFER = '" & REFER & "' AND CVE_CLIE = '" & CVE_PROV & "'),1))"

            cmd.CommandText = SQL
            returnValue = cmd.ExecuteNonQuery().ToString
            If returnValue IsNot Nothing Then
                If returnValue = "1" Then
                    UPDATE_DATOS_CLIE(IMPORTE, NO_FACTURA, CVE_PROV, " - ")
                End If
            End If
    ...
    Private Sub Fg_DoubleClick(sender As Object, e As EventArgs) Handles Fg.DoubleClick
        ...
        End Try
    End Sub
    Private Sub frmRecepcionPagosCxC_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
    End Sub
    Private Sub TPROV_KeyDown(sender As Object, e As KeyEventArgs) Handles TCLIENTE.KeyDown
        Try
    ...
    Private Sub TREFER_KeyDown(sender As Object, e As KeyEventArgs) Handles tREFER.KeyDown
        Try
            ...
            End If
            If e.KeyCode = 13 And SIGUE Then
    ...
    Private Sub tREFER_PreviewKeyDown(sender As Object, e As PreviewKeyDownEventArgs) Handles tREFER.PreviewKeyDown
        Try
            If e.KeyCode = 9 Then
                ...
                Fg.Row = Fg.Rows.Count - 1
                Fg.Col = 1
                _myEditor.StartEditing(Fg.Row, 1, 9)
            End If
    ...
    Private Sub TREFER_Validated(sender As Object, e As EventArgs) Handles tREFER.Validated
        ...
        End Try
    End Sub
    Private Sub TPROV_Validated(sender As Object, e As EventArgs) Handles TCLIENTE.Validated
        Try
...
Public Class MyEditorCxC
    ...
    Public Sub StartEditing(ByVal row As Integer, ByVal col As Integer, Optional ByVal keyRec As Integer = 0)
        ...
        If col = 1 And keyRec = 9 Then
            ...
            _owner.Col = 1
            'frmRecepcionPagosCxC.tMagico.Focus()
            _owner.Select()
            _owner.Select(_row, 1)
            '_owner.StartEditing()
            'SendKeys.Send("{ENTER}")
        ...
        Try
            ...
            If _col = 1 Then
                Try
                    Try
                        IMPORTE = 0 'SALDO
                        If IsNumeric(frmRecepcionPagosCxC.LtDocActual.Text.Replace(",", "")) Then
                            'SALDO
                            IMPORTE = frmRecepcionPagosCxC.LtDocActual.Text.Replace(",", "")
                        End If
                ...
                If col > 1 Then
                    Select Case col
                        Case 1
                            Try
                                Dim DESCR As String
                                DESCR = BUSCA_CAT("ConcCxC", _owner(_owner.Row, 1))

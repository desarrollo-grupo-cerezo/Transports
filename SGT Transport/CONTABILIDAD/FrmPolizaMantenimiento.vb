...
Public Class FrmPolizaMantenimiento
    Dim CADENA As String = ""
    Dim CADENA2 As String = ""
    Dim RUTA_MODELO As String = ""
    Private BindingSource1 As Windows.Forms.BindingSource = New BindingSource
    Private Sub frmPolizaMantenimiento_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frmPolizaMantenimiento_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Poliza de mantenimiento")
    End Sub
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        Try
            ...
            SQL = "SELECT P.CVE_ORD, MAX(O.CVE_UNI) AS CVE_TRAC, ISNULL(MAX(U.DESCR),'') AS DESCR_UNI, MAX(U.CUEN_CONT) AS CTA_UNIDAD, 
                ISNULL(MAX(F.NUM_ALMA),0) AS CVE_ALM_OT, ISNULL(MAX(CVE_ALM),0) AS CVE_ALM_ART, ISNULL(MAX(TIEMPO_REAL),'') AS DOC_OT, SUM(P.IMPORTE) AS IMPORT,
                ISNULL((SELECT SUM(CANT * COSTO * SIGNO) FROM MINVE" & Empresa & " WHERE " & CADENA2 & " AND REFER LIKE '%OT' + P.CVE_ORD + '%'),0) AS RESULT,
                SUM(M.CANT * M.COSTO * M.SIGNO) AS IMPORTE_M
                FROM GCORDEN_TRA_SER_EXT P
                LEFT JOIN MINVE" & Empresa & " M ON M.CVE_ART = P.CVE_ART
                ...
                WHERE ISNULL(A_M,'') <> 'M' AND (P.CVE_ALM = 1 OR P.CVE_ALM= 2 OR P.CVE_ALM = 8) AND 
                O.FECHA >= '" & FSQL(F1.Value) & "' AND O.FECHA <= '" & FSQL(F2.Value) & "' AND 
                O.STATUS <> 'C' AND REFER LIKE '%OT' + P.CVE_ORD + '%'
                GROUP BY P.CVE_ORD ORDER BY P.CVE_ORD"
                ...
                Using dr As SqlDataReader = cmd.ExecuteReader
                    While dr.Read
                        ...
                        If dr("IMPORTE_M") < 0 Then
                            Fg.AddItem("OT" & vbTab & dr("CTA_UNIDAD") & vbTab & "0" & vbTab & dr("CVE_ORD") & vbTab & Math.Abs(dr("IMPORTE_M")) & vbTab & "")
                            SUMA_ALMACEN = SUMA_ALMACEN + Math.Abs(dr("IMPORTE_M"))

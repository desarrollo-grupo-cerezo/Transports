...
Public Class FrmPolizaPagoRef
    ...
    Private Sub FrmPolizaPagoRef_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            TPOLIZA.Text = "Póliza Pago Refaccionesl " & DateTime.Now.ToString("MMMM")
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaPagoRef_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Póliza Pago Refacciones")
    End Sub
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        Try
            SQL = String.Format("SELECT a.*, b.EstaCuadrada, b.TotalDebe, b.TotalHaber, Diferencia = b.TotalDebe - b.TotalHaber 
                                FROM VT_CTB_POLIZA_COMPRAS_REF a
                                INNER JOIN (SELECT	Documento, IIF(SUM(cast(iif(Debe = '', '0.00', Debe) AS decimal(27, 6)))-SUM(cast(iif(Haber = '', '0.00', Haber) AS decimal(27, 6))) = 0, 1, 0) AS EstaCuadrada, 
					                                TotalDebe	= SUM(cast(iif(Debe = '', '0.00', Debe) AS decimal(27, 6))),
					                                TotalHaber	= SUM(cast(iif(Haber = '', '0.00', Haber) AS decimal(27, 6)))
			                                FROM VT_CTB_POLIZA_COMPRAS_REF
			                                WHERE Orden BETWEEN 2 AND 12 AND SubOrden = 0 AND {0}
...
#End Region
                            ...
                            Fg.AddItem("" & vbTab & dr("FechaDocumento") & vbTab & dr("Documento") & vbTab & dr("TipoDocumento") & vbTab & dr("FechaViaje") & vbTab &
                                       ...
                                       dr("ConceptoPolizaDepto") & vbTab & dr("DiaConceptoMov") & vbTab & dr("TipoCambio") & vbTab & IIf(dr("Orden") = "13", Math.Round(dr("TotalDebe"), 2), dr("Debe")) & vbTab & IIf(dr("Orden") = "13", Math.Round(dr("TotalHaber"), 2), dr("Haber")) & vbTab &
                                       dr("CentroCostos") & vbTab & dr("Proyecto"))

                            If Convert.ToInt32(dr("Orden")) > 1 And Convert.ToInt32(dr("Orden")) < 13 And Convert.ToInt32(dr("SubOrden")) = 0 Then

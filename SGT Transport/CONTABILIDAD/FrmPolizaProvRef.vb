...
Public Class FrmPolizaProvRef
    ...
    Private Sub FrmPolizaProvRef_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            TPOLIZA.Text = "Póliza Provisión Refacciones " & DateTime.Now.ToString("MMMM")
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaProvRef_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Póliza Provisión Refacciones")
    End Sub
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        Try
            SQL = String.Format("SELECT a.*, b.EstaCuadrada, b.TotalDebe, b.TotalHaber, Diferencia = b.TotalDebe - b.TotalHaber 
                                FROM VT_CTB_POLIZA_PROVISION_REF a
                                INNER JOIN (SELECT	Documento, IIF(SUM(cast(iif(Debe = '', '0.00', Debe) AS decimal(27, 6)))-SUM(cast(iif(Haber = '', '0.00', Haber) AS decimal(27, 6))) = 0, 1, 0) AS EstaCuadrada, 
			                                ...
			                                FROM VT_CTB_POLIZA_PROVISION_REF
			                                WHERE Orden BETWEEN 2 AND 4 AND SubOrden = 0 AND {0}
...
#End Region
                            ...
                            If Convert.ToInt32(dr("Orden")) > 1 And Convert.ToInt32(dr("Orden")) < 5 And Convert.ToInt32(dr("SubOrden")) = 0 Then

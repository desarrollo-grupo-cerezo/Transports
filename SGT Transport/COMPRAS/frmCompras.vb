...
Public Class frmCompras
    ...
    Private kk As Integer, ENTRA_TXT As Boolean, ENTRA As Boolean, Obser As String = "", Obser_par As String = "", CAPTURA_PAGO_COMPRAS As Integer
    Private ALTA_PROVEEDORES As Integer, ALTA_PRODUCTOS As Integer, T_DOC_ENL As String = "", DOC_ENL As String = "", OBSER_X_DOCUMENTO As Integer = 0
    Private OBSER_X_PARTIDA As Integer = 0, c_ As Integer, INDIRECTOS_X_PARTIDA As Integer, C_IMPUESTOS As Integer, C_ALMACEN As Integer
    Private COMPRA_TOT As String, SERIE_C As String = "", SERIE_DEV As String = "", SERIE_OC As String = "", SERIE_R As String = ""

    Private SERIE_C_TOT As String = "", SERIE_OC_TOT As String = "", SERIE_R_TOT As String = ""
    ...
    Private CVE_ALM_RNBDO As Integer, CVE_ART_RNBDO As String, CVE_ART_NUEVAS As String, CVE_ART_REPARA As String

    Private ACTIVAR_RENOVADO As Integer = 0, ACTIVAR_LLANTAS_NUEVAS As Integer = 0, CVE_CPTO_RENOVADO As Integer = 0, CVE_CPTO_RENOVADO_ENT As Integer = 0

    Private COMPRAS_NEW As Boolean = False, CompEdit As Boolean = 0, TIPO_COMPRA_LOCAL As String, idLlantasNuevas As Guid
    Public Sub New()
        ...
        Try
            ...
            Else
                For Each r As Row In Fg.Rows
                    If r.Index > 0 Then
                        r(0) = r.Index
                        Dim bt As New Button With {.BackColor = SystemColors.Control}
    ...
    Sub CARGA_PARAM_COMPRAS()
        ...
        For k = 1 To 3
            ...
            If NO_ERROR Then
                Exit For
            Else
                Bitacora("frmCompras CARGA_PARAM_COMPRAS  ERRROR AL ABRIR LA TABLA")
            End If
        Next
    End Sub
                ...
                Try
                    ...
                    If CVE_ART.Length > 0 And CANT > 0 Then
                        ...
                        Try
                            ...
                            Else
                                Bitacora("1. PROBLEMA DETECTADO EN GUARDAR NO SE ENCONTRO EL ESQUEMA IMPUESTO frmCOMPRAS " & SQL)
                            End If
                            Reader.Close()
                        Catch ex As Exception
    ...
    Sub BUSCAR_COMPRA(fTIPOC As String, fCVE_DOC As String, fEDIT As String)
        ...
        Try
                    ...
                    Try
                        If TIPO_OTI = 0 Then
                            ...
                            If Obser.Trim.Length = 0 Then
                                Try
                                    If Obser.Trim.Length > 0 Then
                                        SQL = "IF EXISTS (SELECT CLAVE_DOC FROM COMP" & fTIPOC.ToUpper & "_CLIB" & Empresa & " WHERE CLAVE_DOC = '" & fCVE_DOC & "')  
                                        UPDATE COMP" & fTIPOC.ToUpper & "_CLIB" & Empresa & " SET CAMPLIB1 = '" & Obser & "' WHERE CLAVE_DOC = '" & fCVE_DOC & "'
                                     ELSE
                                        INSERT INTO COMP" & fTIPOC.ToUpper & "_CLIB" & Empresa & " (CLAVE_DOC, CAMPLIB1) VALUES('" & fCVE_DOC & "','" & Obser & "')"
            ...
            If Not NO_PAR Then
                Bitacora("PROBLEMA DETECTADO EN BUSCAR_COMPRA NO ENCONTRO LA COMPRA frmCOMPRAS ")
                'Me.Close()
            End If
            ENTRA = True
                    ...
                    If CVE_ART.Length > 0 And CANT > 0 Then
                        ...
                        Try
                            ...
                            If Reader.HasRows Then
                                If Reader.Read Then
                                    CVE_ESQIMPU = Reader("CVE_ESQIMPU")
                                Else
                                    Bitacora("1. PROBLEMA DETECTADO EN CALCULAR IMPORTES NO SE ENCONTRO EL ESQUEMA IMPUESTO frmCOMPRAS ")
                                End If
                            End If
                            Reader.Close()
                        ...
                        Try
                            ...
                            If Reader.Read Then
                                ...
                                IMP4APLA = CInt(Reader("IMP4APLICA"))
                            Else
                                Bitacora("PROBLEMA DETECTADO EN CALCULAR IMPORTES NO ENCONTRO EL IMPUESTO frmCOMPRAS ")
                            End If
                            Reader.Close()
                        Catch ex As Exception
    ...
    Private Sub BarRutaXML_Click(sender As Object, e As EventArgs) Handles BarRutaXML.Click
        ...
        End Try
    End Sub
    Private Sub FrmCompras_Activated(sender As Object, e As EventArgs) Handles Me.Activated
    ...
    Private Sub BarLLantas_Click(sender As Object, e As EventArgs)

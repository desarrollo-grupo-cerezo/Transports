...
Public Class FrmPolizaVentasOtros
    ...
    Private Sub FrmPolizaVentasOtros_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            tPOLIZA.Text = "Poliza ventas otros " & DateTime.Now.ToString("MMMM")
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaVentasOtros_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Poliza ventas otros")
    End Sub
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        If Not Valida_Conexion() Then
            Return
        End If
        Dim SUMA As Decimal = 0, LIB13 As String = "", nDoc As Integer, nPol As Integer
        ...
        Try
            SQL = "SELECT F.CVE_DOC, C.NOMBRE, C.RFC, F.CAN_TOT, ISNULL(F.IMP_TOT1 + F.IMP_TOT4,0) AS IVA, ISNULL(F.IMP_TOT2 + F.IMP_TOT3,0) AS RETENCION, 
                F.IMPORTE AS NETO, ISNULL(L.CAMPLIB2,'') AS CLIB2, 
...
(SELECT TOP 1 CUENT_CONT FROM INVE" & Empresa & " I LEFT JOIN PAR_FACTF" & Empresa & " P ON P.CVE_ART = I.CVE_ART WHERE P.CVE_DOC = F.CVE_DOC) AS CTA_INVE,
                CFD.XML
                ...
                Using dr As SqlDataReader = cmd.ExecuteReader
                    While dr.Read
                        ...
                        If dr("LIB3").ToString.Trim = "S" Then
                            Try
                                nPol = 1
                                Fg.AddItem(nPol & vbTab & dr("CLIB2") & vbTab & "0" & vbTab & dr("CVE_DOC") & " " & dr("RFC") & vbTab & dr("NETO") & vbTab &
                                           "" & vbTab & dr("XML"))

                                Fg.AddItem(nPol + 1 & vbTab & dr("CTA_INVE") & vbTab & "0" & vbTab & dr("CVE_DOC") & " " & dr("RFC") & vbTab & "" & vbTab &
                                           dr("CAN_TOT") & vbTab & " ")

                                If dr("RETENCION") <> 0 Then
                                    Fg.AddItem(nPol + 2 & vbTab & "222-001-0001" & vbTab & "0" & vbTab & dr("CVE_DOC") & " " & dr("RFC") & vbTab &
                                               Math.Abs(dr("RETENCION")) & vbTab & "" & vbTab & " ")
                                End If
                                Fg.AddItem(nPol + 3 & vbTab & "222-001-0001" & vbTab & "0" & vbTab & dr("CVE_DOC") & " " & dr("RFC") & vbTab & "" & vbTab &
                                           dr("IVA") & vbTab & " ")
                                SUMA = SUMA + dr.ReadNullAsEmptyDecimal("NETO")

                                If dr("CLIB2").Trim.Length = 0 Then
    ...
    Private Sub BarExcel_Click(sender As Object, e As ClickEventArgs) Handles BarExcel.Click
        Try
            EXPORTAR_EXCEL_TRANSPORT(Fg, "Poliza ventas otros")
        Catch ex As Exception
            MsgBox("12. " & ex.Message & vbNewLine & ex.StackTrace)
            Bitacora("12. " & ex.Message & vbNewLine & ex.StackTrace)
        End Try
    ...
    Private Sub CboAlmacen_DropDownClosed(sender As Object, e As DropDownClosedEventArgs)
        Try
            ...
            tPOLIZA.Text = "Poliza ventas otros " & MonthName(d1.Month)
        Catch ex As Exception
            Bitacora("14. " & ex.Message & vbNewLine & ex.StackTrace)

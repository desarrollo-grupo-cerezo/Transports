...
Public Class FrmFletesVsGastos
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        SQL = "SELECT A.CVE_VIAJE, A.FECHA, A.CONCILIADO, CASE WHEN A.TIPO_VIAJE = 1 THEN 'Cargado' ELSE 'Vacío' END AS TIP_VIAJE, A.CVE_OPER, A.CVE_TRACTOR, A.FECHA_CARGA,
            ...
            ISNULL((SELECT SUM(IMPORTE) FROM GCLIQ_CASETAS LC LEFT JOIN GCLIQ_PARTIDAS LP ON LP.CVE_LIQ = LC.CVE_LIQ LEFT JOIN GCASIGNACION_VIAJE A2 ON A2.CVE_VIAJE = LP.CVE_VIAJE 
            WHERE A2.CVE_VIAJE = A.CVE_VIAJE AND LP.STATUS <> 'C' AND A2.STATUS <> 'C'),0) AS CASETAS, 
            ...
            ISNULL((SELECT SUM(IMPORTE) FROM GCASIGNACION_VIAJE_GASTOS VG WHERE VG.CVE_VIAJE = A.CVE_VIAJE AND STATUS = 'L' AND ST_GASTOS = 'DEPOSITADO'),0) AS GASTOS,

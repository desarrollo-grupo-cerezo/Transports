...
Public Class FrmPolizaLiq
    ...
    Private Sub FrmPolizaLiq_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            TPOLIZA.Text = "Poliza Liquidación " & DateTime.Now.ToString("MMMM")
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        Try
            SQL = String.Format("SELECT a.*, b.EstaCuadrada, b.TotalDebe, b.TotalHaber, Diferencia = b.TotalDebe - b.TotalHaber 
                                FROM VT_CTB_POLIZA_LIQUIDACIONES a
                                INNER JOIN (SELECT	Documento, IIF(SUM(cast(iif(Debe = '', '0.00', Debe) AS decimal(27, 6)))-SUM(cast(iif(Haber = '', '0.00', Haber) AS decimal(27, 6))) = 0, 1, 0) AS EstaCuadrada, 
			                                ...
			                                FROM VT_CTB_POLIZA_LIQUIDACIONES
			                                WHERE Orden BETWEEN 2 AND 14 AND {0}
                                ...
                                Select Case dr("Orden").ToString()
                                    Case "1"
                                        ...
                                        With poliza
                                            ...
                                            .EsPolizaPrivada = 0
                                            '.UUIDOP
                                            .TipoDocumentoPoliza = EnmTipoDocumentoPoliza.Liquidacion
                                            .Documentos.Add(New Documento(EnmTipoDocumento.Liquidacion, dr("Documento").ToString()))
                                        End With

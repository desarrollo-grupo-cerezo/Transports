...
Public Class FrmPolizaFoliosDinero
    ...
    Private Sub FrmPolizaFoliosDinero_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            tPOLIZA.Text = "Poliza folios dinero Fecha " & DateTime.Now.ToShortDateString
        Catch ex As Exception
    ...
    Sub GET_RUTA_MODELO()
        ...
        End Try
    End Sub
    Private Sub FrmPolizaFoliosDinero_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Me.Dispose()
        CloseTab("Poliza folios dinero")
    End Sub
    ...
    Private Sub BarDesplegar_Click(sender As Object, e As ClickEventArgs) Handles BarDesplegar.Click
        ...
        If Not Valida_Conexion() Then
            Return
        End If
        Dim nDoc As Integer, SUMA As Decimal = 0
        ...
        Try
            SQL = "SELECT GV.FOLIO, ISNULL(O.NOMBRE,'') AS OPERADOR,  ISNULL(GV.IMPORTE,0) AS NETO, 
                ISNULL(C.DESCR,'') AS DESCR_GASTOS, ISNULL(O.CUEN_CONT,'') AS CTA_OPER
                ...
                WHERE GV.ST_GASTOS = 'DEPOSITADO' AND 
                FECHA_DEP >= '" & FSQL(F1.Value) & "' AND FECHA_DEP <= '" & FSQL(F2.Value) & "'
                ORDER BY GV.FOLIO"
            Using cmd As SqlCommand = cnSAE.CreateCommand
                ...
                Using dr As SqlDataReader = cmd.ExecuteReader
                    Do While dr.Read
                        Try
                            CADENA = dr("FOLIO") & " " & dr("DESCR_GASTOS")

                            Fg.AddItem("1" & vbTab & dr("CTA_OPER") & vbTab & "0" & vbTab & CADENA & vbTab & dr("NETO") & vbTab & "" & vbTab & dr("FOLIO"))
                            If dr("CTA_OPER").Trim.Length = 0 Then
                    ...
                    Loop
                End Using
                Fg.AddItem("" & vbTab & "109-001-0012" & vbTab & "0" & vbTab & "" & vbTab & "" & vbTab & SUMA & vbTab & "-")
            End Using
    ...
    Private Sub BarGenPoliza_Click(sender As Object, e As ClickEventArgs) Handles BarGenPoliza.Click
        ...
        Dim CADENA1 As String, FOLIO As String
        Dim UUID_XML As String = "", SERIE As String = ""
        Dim POLIZA_MODELO As String = "", TOTAL As Decimal = 0, NUM_PAR As Long = 12903, NUM_PARX As Long = 0
        ...
        Try
            ...
            s = s & "<ROW VersionCOI=""810"" TipoPoliz=""Op"" DiaPoliz=""30"" ConcepPoliz=""" & tPOLIZA.Text & """><Partidas>"
            'BACKUPTXT("XML", s)
        Catch ex As Exception
        ...
        Try
            For k = 1 To Fg.Rows.Count - 1
                ...
                If CUENTA.Trim.Length > 0 Then
                    ...
                    Try
                        SQL = "UPDATE GCASIGNACION_VIAJE_GASTOS SET POL_GEN = 'S' WHERE FOLIO = '" & FOLIO & "'"
                        Using cmd As SqlCommand = cnSAE.CreateCommand
                            cmd.CommandText = SQL
